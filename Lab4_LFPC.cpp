...
void add_FIRST_A_to_FOLLOW_C(char A, char C) {
    int i;
    for (i = 0; i < TSIZE; ++i) {
        if (i != '^')
            follow[C - 'A'][i] = follow[C - 'A'][i] || first[A - 'A'][i];
    }
}
void add_FOLLOW_A_to_FOLLOW_C(char A, char C) {
    int i;
    for (i = 0; i < TSIZE; ++i) {
        if (i != '^')
            follow[C - 'A'][i] = follow[C - 'A'][i] || follow[A - 'A'][i];
    }
}
void FOLLOW() {
    int t = 0;
            ...
            for (i = 0; i < no_pro; ++i) {
                for (j = 3; j < pro[i].len; ++j) {
                    if (nt == pro[i].str[j]) {
                        ...
                        if (x == pro[i].len)
                            add_FOLLOW_A_to_FOLLOW_C(pro[i].str[0], nt);
                    }
                }
            }
        }
...
void add_FIRST_A_to_FIRST_C(char A, char C) {
    int i;
    for (i = 0; i < TSIZE; ++i) {
        if (i != '^') {
            first[C - 'A'][i] = first[A - 'A'][i] || first[C - 'A'][i];
        }
    }
}
void FIRST() {
    ...
    while (t < no_pro) {
        for (i = 0; i < no_pro; ++i) {
            for (j = 3; j < pro[i].len; ++j) {
                char sc = pro[i].str[j];
                if (isNT(sc)) {
                    add_FIRST_A_to_FIRST_C(sc, pro[i].str[0]);
                    if (first[sc - 'A']['^'])
                        continue;
                }
                else {
...
void add_FIRST_A_to_FIRST_RHS__B(char A, int C) {
    int i;
    for (i = 0; i < TSIZE; ++i) {
        if (i != '^')
            first_rhs[C][i] = first[A - 'A'][i] || first_rhs[C][i];
    }
}
void FIRST_RHS() {
    int i, j;
    ...
    while (t < no_pro) {
        for (i = 0; i < no_pro; ++i) {
            for (j = 3; j < pro[i].len; ++j) {
                char sc = pro[i].str[j];
                if (isNT(sc)) {
                    add_FIRST_A_to_FIRST_RHS__C(sc, i);
                    if (first[sc - 'A']['^'])
                        continue;
                }
                else {
